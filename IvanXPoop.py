...
if password == "IvanXPoop" and username == "IvanXPoop":
    print ("Telah Masuk Sebagai IvanXPoop")
    time.sleep(2)
...
time = int(input("====> $ Masukan Jumlah PACKET   : "))
threads = int(input("====> $ Masukan Jumlah THREADS   : "))
choice = str(input("====> ★ Masukan METHODS   : "))
...
\033[91m     PUNYA IVANXPOOP🌹TRANSFORMER
"""
...
def attack(ip, port, time, threads):
    ...
    print(brand)
    print("\033[92m ★★★★ DARI IVANXPOOP EUYY ★★★★")
...
def attack(ip, port, time, threads):
    ...
    if port is not None:
        port = max(1, min(65535, port))
    print(brand)
    print("\033[92m ★★★★ DARI IVANXPOOP CUY ★★★★")
    fmt = '\033[91m  Sending Attack To ===> Ip {ip}, Port {port}'.format(
        ip=ip,
        port='port {port}'.format(port=port) if port else 'random ports'
    )
